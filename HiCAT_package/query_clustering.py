...
clustering_config
    ...
    informative_modalities : list, tuple, or set
...
from .utils import (
	...
	compute_selected_feature_embedding,
...
class QueryClusteringResult:
    ...
    This dataclass stores the output of query-level multi-modal clustering,
    including the AnnData object with predicted cluster labels, the integrated
    multi-modal embedding used for clustering, modality-specific reduced
    embeddings, and the clustering configuration.

    Attributes
    ----------
    adata : AnnData
        Query AnnData object containing the clustering results.

        This object is usually copied from the first selected modality AnnData
        object after row alignment. The predicted cluster labels are stored in
        `adata.obs[pred_key]`, and the integrated multi-modal embedding is stored
        in `adata.obsm["X_integrated_multimodal"]`.

    pred_df : pandas.DataFrame
        DataFrame containing spot/cell-level clustering predictions.

        The index matches `adata.obs_names`. It usually contains at least one
    ...
    selected_modalities : list of str
        ...
        `clustering_config["informative_modalities"]`.
    ...
    config : dict
        ...
        -------
        {
            "informative_modalities": ["Gene", "Image"],
            "reduce_dimension_approach": "PCA",
            "pcs_num_dic": {
                "Gene": 30,
                "Image": 20,
            },
...
def compute_query_integrated_embedding(
    ...
    clustering_config : dict
        ...
        Required when ``reduce_dimension_approach='pca'``:
        ...
        Required when ``reduce_dimension_approach='selected_features'``:
    ...
    if "selected_modalities" not in clustering_config:
        raise KeyError("clustering_config must contain 'selected_modalities'.")

    selected_modalities = list(clustering_config["selected_modalities"])
    dim_reduction_method = clustering_config.get("dim_reduction_method", "pca")
...
def query_multi_modal_clustering(
    ...
    pred_key : str, default="query_cluster"
        Column name used to store predicted cluster labels in
        ``result.adata.obs[pred_key]``.
    ...
    store_obsm : bool, default=True
        Whether to store embeddings in the returned AnnData object.

        If True, the following fields are added to ``result.adata.obsm``:

        - ``"X_integrated_multimodal"``:
          concatenated multi-modal embedding used for clustering.
        - ``"X_gene_reduced"``:
          reduced Gene embedding, if Gene is selected.
        - ``"X_image_reduced"``:
          reduced Image embedding, if Image is selected.
        - ``"X_protein_reduced"``:
          reduced Protein embedding, if Protein is selected.
    ...
    copy : bool, default=True
        Whether to copy the base AnnData object before adding cluster labels
        and embeddings.

        If True, the returned AnnData object is a copy. If False, the function
        modifies the base AnnData object returned by
        ``compute_query_integrated_embedding``.
    ...
    QueryClusteringResult
        ...
        ``adata`` : AnnData
            Query AnnData object with predicted cluster labels stored in
            ``adata.obs[pred_key]``. If ``store_obsm=True``, integrated and
            modality-specific embeddings are also stored in ``adata.obsm``.
    ...
    adata_out = base_adata.copy() if copy else base_adata
    ...
    adata_out.obs[pred_key] = pd.Categorical(cluster_labels.astype(str))

    if store_obsm:
        adata_out.obsm["X_integrated_multimodal"] = integrated_embedding
        for modality, embedding in modality_embedding_dic.items():
            adata_out.obsm[f"X_{modality.lower()}_reduced"] = embedding

    pred_df = pd.DataFrame(
        {
            "spot_id": adata_out.obs_names,
            pred_key: adata_out.obs[pred_key].astype(str).to_numpy(),
        },
    ...
    if print_results:
        ...
        print(adata_out.obs[pred_key].value_counts())
...
def identify_boundary_cluster(
    ...
    candidate_feature_sets: Sequence[Sequence[str]] = (
        ("rgb_0", "rgb_1", "rgb_2"),
    ...
    var_names = set(image_adata.var_names.astype(str))

    if boundary_features is None:
        selected_features = None
        for feature_set in candidate_feature_sets:
            feature_set = list(feature_set)
            if all(feature in var_names for feature in feature_set):
        ...
        selected_features = list(boundary_features)
        missing_features = [feature for feature in selected_features if feature not in var_names]
        if len(missing_features) > 0:
            raise KeyError(
                f"boundary_features not found in image_adata.var_names: {missing_features}"
            )
...
def postprocess_query_clustering_result(
    ...
    copy : bool, default=True
        Whether to copy ``result.adata`` before adding postprocessed columns.
    print_results : bool, default=True
    ...
    adata_out = result.adata.copy() if copy or result.adata.is_view else result.adata
    active_pred_key = pred_key
    postprocess_info: Dict[str, Any] = {}

    if pred_key not in adata_out.obs.columns:
        raise KeyError(f"Missing column in result.adata.obs: {pred_key}")
    ...
    pred_df = pd.DataFrame({"spot_id": adata_out.obs_names}, index=adata_out.obs_names)
    pred_df[pred_key] = adata_out.obs[pred_key].astype(str).to_numpy()

    if active_pred_key != pred_key:
        pred_df[active_pred_key] = adata_out.obs[active_pred_key].astype(str).to_numpy()
    ...
    return result.__class__(
        adata=adata_out,
        pred_df=pred_df,
        cluster_labels=adata_out.obs[active_pred_key].astype(str).to_numpy(),
...
"""
# usage
result = query_multi_modal_clustering(
    query_adata_dic=query_adata_dic,
    clustering_config=clustering_config,
    pred_key="query_cluster",
)

result = postprocess_query_clustering_result(
    result=result,
    query_adata_dic=query_adata_dic,
    pred_key="query_cluster",
    boundary_refinement_config={
...
final_key = result.config["final_pred_key"]
result.adata.obs[final_key]
